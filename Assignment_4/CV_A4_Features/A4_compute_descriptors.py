...
def CNN_BoVW(sift_features, cnn_features):
    ...
    bovw_path = f'./A4_2020312552_cnn_bovw_{n_clusters}.pkl'
    bovw = load_object(bovw_path)
    if bovw is None:
        kmeans_path = f'./A4_2020312552_cnn_kmeans_{n_clusters}.pkl'
...
def CNN_VLAD(sift_features, cnn_features):
    ...
    """
    n_clusters = 2
    kmeans_path = f'./A4_2020312552_cnn_kmeans_{n_clusters}_for_vlad.pkl'
    kmeans = load_object(kmeans_path)
    if kmeans is None:
        kmeans = kmeans_clustering(cnn_features.reshape(-1, 14*14, 512), n_clusters)
        save_object(kmeans_path, kmeans)
...
def SIFT_BoVW(sift_features, cnn_features):
    ...
    bovw_path = f'./A4_2020312552_sift_bovw_{n_clusters}.pkl'
    bovw = load_object(bovw_path)
    if bovw is None:
        kmeans_path = f'./A4_2020312552_sift_kmeans_{n_clusters}.pkl'
        kmeans = load_object(kmeans_path)
        if kmeans is None:
    ...
    if use_pca:
        ...
        pca_path = f'./A4_2020312552_sift_pca_{n_clusters}_to_{target_components}.pkl'
...
def SIFT_VLAD(sift_features, cnn_features):
    ...
    """
    n_clusters = 32
    kmeans_path = f'./A4_2020312552_sift_kmeans_{n_clusters}_for_vlad.pkl'
    kmeans = load_object(kmeans_path)
    if kmeans is None:
        kmeans = kmeans_clustering(sift_features, n_clusters)
        save_object(kmeans_path, kmeans)
...
def SC_all(sift_features, cnn_features):
    ...
    cnn_features.shape = (2000, 14, 14, 512)
    """
    desc_path = f'./A4_2020312552_SC_all.pkl'
...
if __name__ == '__main__':
    ...
    # descriptor 저장하기
    save_descriptors('./A4_2020312552.des', descriptor)